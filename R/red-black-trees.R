...
pmatch::`:=`(rbt_set, RBT_SET_EMPTY | RBT_SET(col : rbt_colours,
                                              val,
                                              left : rbt_set,
                                              right : rbt_set))
pmatch::`:=`(rbt_map, RBT_MAP_EMPTY | RBT_MAP(col : rbt_colours,
                                              key, val,
                                              left : rbt_map,
                                              right : rbt_map))
...
rbt_set_insert_ <- function(tree, elm) {
    ...
    if (elm < tree$val)
        rbt_set_balance(RBT_SET(tree$col, tree$val, rbt_set_insert_(tree$left, elm), tree$right))
    else if (elm > tree$val)
        rbt_set_balance(RBT_SET(tree$col, tree$val, tree$left, rbt_set_insert_(tree$right, elm)))
    else
...
rbt_map_balance <- function(tree) { # fixme: add deletion transformations
    pmatch::cases(
        tree,
        RBT_MAP(RBT_BLACK,zkey,zval,RBT_MAP(RBT_RED,xkey,xval,a,RBT_MAP(RBT_RED,ykey,yval,b,c)),d) ->
            RBT_MAP(RBT_RED,ykey,yval,RBT_MAP(RBT_BLACK,xkey,xval,a,b),RBT_MAP(RBT_BLACK,zkey,zval,c,d)),
        RBT_MAP(RBT_BLACK,zkey,zval,RBT_MAP(RBT_RED,ykey,yval,RBT_MAP(RBT_RED,xkey,xval,a,b),c),d) ->
            RBT_MAP(RBT_RED,ykey,yval,RBT_MAP(RBT_BLACK,xkey,xval,a,b),RBT_MAP(RBT_BLACK,zkey,zval,c,d)),
        RBT_MAP(RBT_BLACK,xkey,xval,a,RBT_MAP(RBT_RED,ykey,yval,b,RBT_MAP(RBT_RED,zkey,zval,c,d))) ->
            RBT_MAP(RBT_RED,ykey,yval,RBT_MAP(RBT_BLACK,xkey,xval,a,b),RBT_MAP(RBT_BLACK,zkey,zval,c,d)),
        RBT_MAP(RBT_BLACK,xkey,xval,a,RBT_MAP(RBT_RED,zkey,zval,RBT_MAP(RBT_RED,ykey,yval,b,c),d)) ->
            RBT_MAP(RBT_RED,ykey,yval,RBT_MAP(RBT_BLACK,xkey,xval,a,b),RBT_MAP(RBT_BLACK,zkey,zval,c,d)),
        otherwise -> tree)
}


rbt_map_insert_ <- function(tree, key, val) {
    if (is_red_black_map_empty(tree))
        return(RBT_MAP(RBT_RED, key, val, RBT_MAP_EMPTY, RBT_MAP_EMPTY))

    if (key < tree$key)
        rbt_map_balance(RBT_MAP(tree$col, tree$key, tree$val, rbt_map_insert_(tree$left, key, val), tree$right))
    else if (key > tree$key)
        rbt_map_balance(RBT_MAP(tree$col, tree$key, tree$val, tree$left, rbt_map_insert_(tree$right, key, val)))
    else
        tree # the value is already in the tree, at this level, so just return
}
...
rbt_map_member <- function(tree, k) {
    ...
    pmatch::cases(
        ...
        RBT_MAP(col, key, val, left, right) -> {
            if (key == k) t
            else if (key > k) rbt_map_member(left, k)
            else rbt_map_member(right, k)
        }
    )
}
